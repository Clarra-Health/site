            ...
            item && (item as any).children ? (
              <div key={item.to} className="relative">
                <NavigationMenu>
                  <NavigationMenuList>
                    <NavigationMenuItem>
                      <NavigationMenuTrigger className="bg-transparent p-0 hover:bg-transparent text-muted-foreground data-[state=open]:text-primary text-lg">
                        {item.label}
                        ...
                        <div className="w-[260px] p-2">
                          ...
                          {(item as any).children.map((child: any) => (
                            <Link
                              key={child.to}
                              to={child.to}
                              className="block rounded-md px-3 py-2 text-lg text-foreground hover:bg-accent hover:text-accent-foreground"
                            >
                              {child.label}
                            </Link>
                          ))}
            ...
            {nav.map((item) => (
              <div key={item.to}>
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `block rounded-md px-3 py-2 text-lg transition-colors ${isActive ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-primary hover:text-primary-foreground"}`
                  }
                >
                  {item.label}
                </NavLink>
                  ...
                  <div className="ml-3 mt-1 space-y-1">
                    {(item as any).children.map((child: any) => (
                      <NavLink
                        ...
                        onClick={() => setOpen(false)}
                        className={({ isActive }) =>
                          `block rounded-md px-3 py-2 text-lg transition-colors ${isActive ? "bg-accent text-accent-foreground" : "text-muted-foreground hover:bg-accent hover:text-accent-foreground"}`
                        }
                      >
                        {child.label}
                      </NavLink>
